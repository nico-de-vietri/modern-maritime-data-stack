...
with silver as(
    ...
    left join {{ ref('ports') }} p1 on e.destination_clean = p1.port_name

    -- Fallback: match on country & location code
    left join {{ ref('ports') }} p2 on e.dest_country_guess = p2.country_code and e.dest_loc_guess = p2.location_code
    left join {{ ref('country_code') }} cc1 on p1.country_code=cc1.country_code
    left join {{ ref('navigation_status') }} nv1 on e.navigational_status=nv1.navigation_status
)
