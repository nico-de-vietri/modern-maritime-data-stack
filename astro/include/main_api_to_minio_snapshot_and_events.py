...
async def process_and_upload_factory():
    ...
    ensure_bucket(minio_client, BUCKET_SNAPSHOT_NAME)
    ensure_bucket(minio_client, BUCKET_EVENTS_NAME)
    batch_uploader_snapshot = BatchUploader(minio_client, BUCKET_SNAPSHOT_NAME)
    batch_uploader_events = BatchUploader(minio_client, BUCKET_EVENTS_NAME)

    async def process_and_upload(position_data):
        await batch_uploader_snapshot.add_and_upload(position_data)

        # upload individual event as JSON (append-only)
        event_data = json.dumps(position_data).encode("utf-8")
        filename = generate_object_name(prefix="event_")
        upload_json(minio_client, BUCKET_EVENTS_NAME, filename, event_data)
